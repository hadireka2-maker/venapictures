...
import { updateLead as supaUpdateLead } from '../services/leads';
...
export const Leads: React.FC<LeadsProps> = ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (modalMode === 'add' || modalMode === 'edit') {
            ...
            // Persist to Supabase (best-effort) while keeping UI responsive
            try {
                ...
                await supaCreateProject({
                    ...
                    dp_proof_url: null,
                    promo_code_id: newProject.promoCodeId,
                    discount_amount: newProject.discountAmount || null,
                    transport_cost: null,
                ...
                if (dpAmount > 0 && formData.dpDestinationCardId) {
                    await supaCreateTransaction({
                        ...
                        card_id: formData.dpDestinationCardId,
                        pocket_id: null,
                        project_id: newProject.id,
                    } as any);
                }
                if (promoCode) {
                    await supaIncrementPromoUsage(promoCode.id);
                }
                await supaUpdateLead(selectedLead.id, { status: 'Dikonversi', date: new Date().toISOString() } as any);
            } catch (err) {
